...
class MultiHeadAttention(nn.Module):
    """ multiple heads of self-attention in parallel and concatinate the results """
...
class FeedFoward(nn.Module):
    """ 
    a simple linear layer followed by a non-linearity MLP Multi Layer Perceptron
    After self-attention each token individually is added to feed forward to e.g. 
    think ahead of what they just learned in self-attention
    """
...
class Block(nn.Module):
    ...
    def forward(self, x):
        # add
        x = x + self.sa(self.ln1(x)) # resitual connections / additional compuation forked off and returned
